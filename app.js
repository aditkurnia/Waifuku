...
function saveApiIndex(i){apiKeyIndex=i%API_KEYS.length;localStorage.setItem(API_INDEX_KEY,String(apiKeyIndex))}
...
async function callModel(tail){if(isOfflineLocked())throw new Error('OFFLINE_LOCKED');const text=systemPrompt()+'\n\nPercakapan:\n'+convoText()+tail;const order=[apiKeyIndex,(apiKeyIndex+1)%API_KEYS.length];for(let k=0;k<order.length;k++){try{const idx=order[k];const out=await geminiCallWithKey(idx,text);if(apiKeyIndex!==idx)saveApiIndex(idx);setStatusUI('online');return out}catch(e){}}lockOffline();throw new Error('BOTH_KEYS_FAILED')}
